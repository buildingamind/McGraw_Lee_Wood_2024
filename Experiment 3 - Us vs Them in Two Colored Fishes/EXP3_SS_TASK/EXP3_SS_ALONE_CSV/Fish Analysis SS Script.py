...
with warnings.catch_warnings():
    ...
    for model in models:
        # Load data
        DATA_PATH = "G:/McGraw 2024 Source/Experiment 3 - Us vs Them in Two Colored Fishes/EXP3_SS_TASK/EXP3_SS_ALONE_CSV/"
        source_csv = fr"ALONE_SS_{model}_1.0.csv"
        ...
        plt.title(f'Self-Grouping Task from Socially Reared Agents\n({model} = 1.0)  \n', fontweight='bold',
                fontsize=15)
        plt.savefig(rf'{DATA_PATH}/ALONE_{model}_distance.png')
        plt.close()
        ...
        output_df.to_csv(rf"{DATA_PATH}/ALONE_FISH_{model}_Self_Segregation_by_Episode.csv")
        ###############################################################################
        ...
        # Describe the results of the T-Test and significance level
        print(f"Orange to Blue vs. Orange to Orange and Blue to Blue: {ttest_results}")
        with open(rf"{DATA_PATH}/ALONE_FISH_{model}_Self_Segregation_TTest.txt", "w") as f:
        ...
        plt.savefig(f'{DATA_PATH}/ALONE_{model}_SS.png')
        plt.close()
        ...
        plt.savefig(f'{DATA_PATH}/ALONE_{model}_SS_WITHIN.png')
        plt.close()
        ...
        individual_data_df.to_csv(rf"{DATA_PATH}/ALONE_FISH_{model}_Self_Segregation_Individual_Data.csv", index=False)
