...
DATA_PATH = fr"G:/McGraw 2024 Source/Experiment 3 - Us vs Them in Two Colored Fishes/EXP3_2AFC_TASK/EXP3_2AFC_ALONE_CSV"
...
for model_name, model_data in models.items():
    ...
    for file_name, agent_data in model_data.items():
        ...
        with open(rf"{DATA_PATH}/ALONE_FISH_{model_name}_{agent_name}_2AFC_TTest.txt", "w") as f:
    ...
    results_df.to_csv(f"{DATA_PATH}/ALONE_{model_name}_2AFC_SocialPreference_by_Episode.csv", index=False)
    print(f"Results Logged to {DATA_PATH}/ALONE_{model_name}_2AFC_SocialPreference_by_Episode.csv")

    # Generate the plot with error bars
    fig, ax = plt.subplots(figsize=(12, 6))

    # With this inverted social score, we can emphasize the direction of the social score more clearly
    # To do this, we need to convert the scale to show directionality, and also scale from -1 to 1
    ...
    save_name = f"2AFC_ALONE_{model_name}.png"
