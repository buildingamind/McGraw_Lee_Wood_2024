...
DATA_PATH = fr"G:/McGraw 2024 Source/Experiment 3 - Us vs Them in Two Colored Fishes/EXP3_2AFC_TASK/EXP3_2AFC_LOWIM_CSV"
...
for model_name, model_data in models.items():
    models[model_name] = {
        f"OrangeAgent_{model_name}_01_0.001.csv": None,
        f"OrangeAgent_{model_name}_02_0.001.csv": None,
        f"OrangeAgent_{model_name}_03_0.001.csv": None,
        f"OrangeAgent_{model_name}_04_0.001.csv": None,
        f"BlueAgent_{model_name}_01_0.001.csv": None,
        f"BlueAgent_{model_name}_02_0.001.csv": None,
        f"BlueAgent_{model_name}_03_0.001.csv": None,
        f"BlueAgent_{model_name}_04_0.001.csv": None,
    }
...
for model_name, model_data in models.items():
    ...
    for file_name, agent_data in model_data.items():
        agent_data = pd.read_csv(f"{DATA_PATH}/{file_name}", on_bad_lines='skip', nrows=500000).iloc[:, :-1]  #
        agent_name = file_name.split("_")[0]
        agent_number = re.search(r'_(\d{2})_', file_name).group(1)
        ...
        with open(rf"{DATA_PATH}/LOWIM_FISH_{model_name}_{agent_name}_2AFC_TTest.txt", "w") as f:
    ...
    results_df.to_csv(f"{DATA_PATH}/LOW_{model_name}_2AFC_SocialPreference_by_Episode.csv", index=False)
    print(f"Results Logged to {DATA_PATH}/LOW_{model_name}_2AFC_SocialPreference_by_Episode.csv")
    ...
    ax.bar([k.replace('Agent_0', '') for k in agents_names], preference_scores_inverted, yerr=preference_scores_std, color=['darkorange']*4 + ['royalblue']*4,
    ...
    save_name = DATA_PATH + f"/2AFC_LOWIM_{model_name}.png"
